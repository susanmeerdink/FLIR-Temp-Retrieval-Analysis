...
%   will also contain the visible image stored in the file (only for image
%   taken in MSX, Picture-in-Picture, or thermal fusion mode).
%
%   Example:
...
elseif strcmp(fType,'jpg') % for radiometric .jpg files
    ...
    elseif strcmp(fileInfo.thermalFormat,'JPG') % image containing only visible data
        ...
        fid = fopen('temporaryVis.jpg','w'); % write visible PNG to temporary file
        fwrite(fid,vis);
        fclose(fid);
        clear vis
        
        vis = imread('temporaryVis.jpg'); % read in temporary file and then delete file
        if ispc
            fullFileName = strcat('"',pwd,'\temporaryVis.jpg"');
            system(horzcat('DEL ',fullFileName));
        elseif isunix
            fullFileName = strcat(pwd,'/temporaryVis.jpg');
            fullFileName = unixFN(fullFileName);
            system(horzcat('rm ',fullFileName));
        end
        pause(0.0001);% let system catch up with itself
        
        fileInfo.visImage = vis; % store visible image
